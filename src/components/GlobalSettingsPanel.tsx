...
import { Bell, CircleSlash2, EyeOff, Flag, MessageCircle, Shield, UserRoundPlus, UsersRound, Volume2, X } from 'lucide-react';
...
export default function GlobalSettingsPanel({ isDark, theme, setTheme, coins, setCoins }: { isDark: boolean; theme: 'dark' | 'light'; setTheme: (theme: 'dark' | 'light') => void; coins: number; setCoins: Dispatch<SetStateAction<number>> }) {
  ...
  const moderationRole = (() => {
    ...
    return role === 'member' || role === 'moderator' || role === 'admin' ? role : 'member';
  })() as ModerationRole;
  ...
  return <section className="space-y-4 border-t border-white/5 pt-6 text-left">
    ...
    {(moderationRole === 'moderator' || moderationRole === 'admin') && <ModerationPanel role={moderationRole} />}
    {(moderationRole === 'moderator' || moderationRole === 'admin') && <DeveloperCenter role={moderationRole} />}
...
function Switch({ label, checked, onChange }: { label: string; checked: boolean; onChange: () => void }) { return <button type="button" role="switch" aria-checked={checked} onClick={onChange} className="flex min-h-11 w-full items-center justify-between rounded-xl border border-[var(--axo-border)] px-3 py-2 text-left text-[10px] font-bold"><span>{label}</span><span aria-hidden="true" className={`h-5 w-9 rounded-full p-0.5 transition ${checked ? 'bg-[#FF2D55]' : 'bg-zinc-500'}`}><span className={`block h-4 w-4 rounded-full bg-white transition ${checked ? 'translate-x-4' : ''}`} /></span></button>; }
...
function Dialog({ title, children, onClose }: { title: string; children: ReactNode; onClose: () => void }) { const titleId = `settings-dialog-${title.toLowerCase().replace(/[^a-z0-9]+/g, '-')}`; return <div className="fixed inset-0 z-[70] flex items-center justify-center bg-[var(--axo-overlay)] p-4"><div role="dialog" aria-modal="true" aria-labelledby={titleId} className="w-full max-w-sm rounded-[28px] border border-[var(--axo-border)] bg-[var(--axo-surface-strong)] p-6 text-[var(--axo-text)] shadow-2xl"><div className="flex items-center justify-between"><h3 id={titleId} className="text-sm font-black">{title}</h3><button type="button" onClick={onClose} aria-label="Fermer"><X className="h-5 w-5" /></button></div><div className="mt-4 text-xs leading-relaxed text-[var(--axo-text-muted)]">{children}</div></div></div>; }

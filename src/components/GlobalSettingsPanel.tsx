...
import AdvancedAccountPanel from './AdvancedAccountPanel';
...
const people = [
  { id: 'lena', name: 'Lena X', username: '@lena_x' },
...
export default function GlobalSettingsPanel({ isDark, theme, setTheme, coins, setCoins }: { isDark: boolean; theme: 'dark' | 'light'; setTheme: (theme: 'dark' | 'light') => void; coins: number; setCoins: Dispatch<SetStateAction<number>> }) {
  ...
  const [muted, setMuted] = useState<string[]>(['neon']);
  const [requests, setRequests] = useState(['Mila K.', 'Dorian M.']);
  ...
  const setLanguageAndPersist = (value: string) => { setLanguage(value); localStorage.setItem('axo_language', value); };
  return <section className="space-y-4 border-t border-white/5 pt-6 text-left">
    <div><h3 className="text-[10px] font-black uppercase tracking-widest text-[#A855F7]">Centre de paramètres</h3><p className="mt-1 text-[11px] text-zinc-400">Préférences locales de cette maquette.</p></div>
    <div className={`rounded-2xl border p-4 ${panel}`}><h4 className="flex items-center gap-2 text-xs font-black"><Bell className="h-4 w-4 text-[#FF2D55]" />Notifications et langue</h4><div className="mt-3 space-y-2">{([['push', 'Notifications push'], ['email', 'Résumé e-mail'], ['sound', 'Sons de l’application']] as const).map(([key, label]) => <div key={key}><Switch label={label} checked={prefs[key]} onChange={() => toggle(key)} /></div>)}<label className="mt-3 block text-[10px] font-bold text-zinc-400">Langue<select value={language} onChange={event => setLanguageAndPersist(event.target.value)} className="mt-1 w-full rounded-xl border border-white/10 bg-transparent px-3 py-2 text-xs outline-none"><option>Français</option><option>English</option><option>Lingala</option></select></label></div></div>
    <div className={`rounded-2xl border p-4 ${panel}`}><h4 className="flex items-center gap-2 text-xs font-black"><Globe2 className="h-4 w-4 text-cyan-400" />Apparence et accessibilité</h4><div className="mt-3 grid grid-cols-2 gap-2"><button type="button" onClick={() => setTheme('light')} className={`rounded-xl border px-3 py-2 text-[10px] font-bold ${theme === 'light' ? 'border-cyan-400 text-cyan-500' : 'border-white/10'}`}>Clair</button><button type="button" onClick={() => setTheme('dark')} className={`rounded-xl border px-3 py-2 text-[10px] font-bold ${theme === 'dark' ? 'border-cyan-400 text-cyan-500' : 'border-white/10'}`}>Sombre</button></div><div className="mt-2 space-y-2"><Switch label="Réduire les animations" checked={prefs.reduced} onChange={() => toggle('reduced')} /><Switch label="Contraste renforcé" checked={prefs.contrast} onChange={() => toggle('contrast')} /></div></div>
...
function Switch({ label, checked, onChange }: { label: string; checked: boolean; onChange: () => void }) { return <button type="button" onClick={onChange} className="flex w-full items-center justify-between rounded-xl border border-white/5 px-3 py-2 text-left text-[10px] font-bold"><span>{label}</span><span className={`h-5 w-9 rounded-full p-0.5 transition ${checked ? 'bg-[#FF2D55]' : 'bg-zinc-600'}`}><span className={`block h-4 w-4 rounded-full bg-white transition ${checked ? 'translate-x-4' : ''}`} /></span></button>; }
function PrivacySelect({ label, value, options, onChange }: { label: string; value: string; options: string[]; onChange: (value: string) => void }) { return <label className="block text-[10px] font-bold text-zinc-400">{label}<select value={value} onChange={event => onChange(event.target.value)} className="mt-1 w-full rounded-xl border border-white/10 bg-transparent px-3 py-2 text-xs text-inherit">{options.map(option => <option key={option}>{option}</option>)}</select></label>; }
function PeopleGroup({ title, icon, ids, action, onAction, empty }: { title: string; icon: ReactNode; ids: string[]; action: string; onAction: (id: string) => void; empty: string }) { return <div><p className="flex items-center gap-1 text-[10px] font-black">{icon}{title}</p>{ids.length ? ids.map(id => { const person = people.find(item => item.id === id)!; return <div key={id} className="mt-2 flex items-center justify-between rounded-xl border border-white/5 px-3 py-2 text-[10px]"><span><b>{person.name}</b> <span className="text-zinc-500">{person.username}</span></span><button type="button" onClick={() => onAction(id)} className="font-bold text-cyan-400">{action}</button></div>; }) : <p className="mt-2 text-[10px] text-zinc-500">{empty}</p>}</div>; }
function Dialog({ title, children, onClose }: { title: string; children: ReactNode; onClose: () => void }) { return <div className="fixed inset-0 z-[70] flex items-center justify-center bg-black/75 p-4"><div role="dialog" aria-modal="true" className="w-full max-w-sm rounded-[28px] bg-zinc-900 p-6 text-white shadow-2xl"><div className="flex items-center justify-between"><h3 className="text-sm font-black">{title}</h3><button type="button" onClick={onClose} aria-label="Fermer"><X className="h-5 w-5" /></button></div><div className="mt-4 text-xs leading-relaxed text-zinc-300">{children}</div></div></div>; }

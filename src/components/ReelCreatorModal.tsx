...
export default function ReelCreatorModal({ open, onClose, onPublish, currentUser }: ReelCreatorModalProps) {
  ...
  const publish = () => {
    ...
    setState('publishing');
    window.setTimeout(() => {
      onPublish({ id: `reel-${Date.now()}`, creatorName: currentUser.name, creatorUsername: currentUser.username.replace('@', ''), avatar: currentUser.avatar, mediaUrl: videoPreview || coverPreview, mediaType: videoPreview ? 'video' : 'image', posterUrl: videoPreview ? coverPreview : undefined, caption: caption.trim() || 'Nouveau Reel Axora', likes: 0, commentsCount: 0, shares: 0, musicTrack: 'Audio original', isVerified: false, comments: [] });

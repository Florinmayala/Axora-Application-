...
function Modal({ title, children, onClose }: { title: string; children: ReactNode; onClose: () => void }) { const titleId = `security-dialog-${title.toLowerCase().replace(/[^a-z0-9]+/g, '-')}`; return <div className="fixed inset-0 z-[70] flex items-center justify-center bg-[var(--axo-overlay)] p-4"><div role="dialog" aria-modal="true" aria-labelledby={titleId} className="w-full max-w-sm rounded-[28px] border border-[var(--axo-border)] bg-[var(--axo-surface-strong)] p-6 text-[var(--axo-text)] shadow-2xl"><div className="flex items-center justify-between gap-3"><h3 id={titleId} className="text-sm font-black">{title}</h3><button type="button" onClick={onClose} aria-label="Fermer"><X className="h-5 w-5" /></button></div><div className="mt-4 text-xs leading-relaxed text-[var(--axo-text-muted)]">{children}</div></div></div>; }
